...
train_images = (
    train_images.shuffle(2000).batch(batch_size).cache().prefetch(AUTOTUNE)
)

validation_images = (
    validation_images.batch(batch_size).cache().prefetch(AUTOTUNE)
)
...
def create_model(hp):
    ...
    new_model = models.Sequential(
        [
            ...
            layers.MaxPooling2D((2, 2)),
            layers.Conv2D(
                32, (3, 3), activation='relu', kernel_regularizer=regularizer
            ),
